...
namespace MySpace.DataMining.DistributedObjects
{
    public class Exec
    {
        ...
        public static string DDShell(string line, bool suppresserrors, bool step)
        {
            ...
            else if (line.StartsWith("qizmt", StringComparison.OrdinalIgnoreCase)
                    || line.StartsWith("\"qizmt", StringComparison.OrdinalIgnoreCase))
            {
                int isp = line.IndexOf(' ');
                if (step)
                {
                    line = "qizmt -debug-step " + line.Substring(isp + 1);
                }
                else
                {
                    line = "qizmt -debug " + line.Substring(isp + 1);
                }
            }
