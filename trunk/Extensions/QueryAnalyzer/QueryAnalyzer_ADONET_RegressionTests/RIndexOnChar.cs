...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    public partial class Program
    {
        ...
        private static OneRow[] RIndexOnChar_PrepareSourceTable(string sourcetablename, int testsize)
        {
...
taking a break.".Split(new string[] { " ", Environment.NewLine}, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnChar()
        {
            ...
            testrows = RIndexOnChar_PrepareSourceTable(tablename, TESTSIZE);
      ...
      <MapReduce>
        ...
        <Reduce>
          <![CDATA[
            ...
            {
                Console.WriteLine("Creating RIndexes on key column = str...");
                DbConnection conn = fact.CreateConnection();
                conn.ConnectionString = "Data Source = localhost";
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname_str + " FROM " + tablenameSorted_str + " ON str";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname_pin + " FROM " + tablenameSorted_str + " PinMemory ON str";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                {
                    List<OneRow> results = new List<OneRow>();
                    cmd.CommandText = "rselect * from " + indexname_pin.ToUpper() + " where key = '" + key.Replace("'", "''") + "'";
                    DbDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        OneRow r;
