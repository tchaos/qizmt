...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        public static void DbAggregators_LAST(string tablename)
        {
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine("Testing LAST(datetime)...");
                        DbCommand cmd = conn.CreateCommand();
                        cmd.CommandText = "select id, LAST(bday) from " + tablename + " group by id";
                        ...
                        while (reader.Read())
                        {
                            ...
                            {
                                throw new Exception("id returned from LAST group by is invalid.");
                            }
                            bool found = false;
                            ...
                            if (!found)
                            {
                                throw new Exception("Invalid LAST received: " + first.ToString() + " instead.");
                            }
