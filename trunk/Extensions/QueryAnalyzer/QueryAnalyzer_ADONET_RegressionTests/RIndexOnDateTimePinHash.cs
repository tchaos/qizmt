...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        private static OneRow[] RIndexOnDateTimePinHash_PrepareSourceTable(string sourcetablename, int testsize)
        {
...
taking a break.".Split(new string[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnDateTimePinHash()
        {
            ...
            testrows = RIndexOnDateTimePinHash_PrepareSourceTable(tablename, TESTSIZE);
            ...
            {
                Console.WriteLine("Creating RIndexes pinmemoryhash on key column = datetime...");
                DbConnection conn = fact.CreateConnection();
                conn.ConnectionString = "Data Source = localhost";
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname + " FROM " + tablenameSorted + " pinmemoryhash ON DT";
                cmd.ExecuteNonQuery();
                conn.Close();
