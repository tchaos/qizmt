...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        private static OneRow[] RIndexOnLong_PrepareSourceTable(string sourcetablename, int testsize)
        {
...
taking a break.".Split(new string[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnLong()
        {
            ...
            {
                Console.WriteLine("Creating RIndexes on key column = long...");
                DbConnection conn = fact.CreateConnection();
                conn.ConnectionString = "Data Source = localhost";
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname + " FROM " + tablenameSorted + " ON NUM3";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname_pin + " FROM " + tablenameSorted + " PinMemory ON NUM3";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                cmd.ExecuteNonQuery();
                cmd.CommandText = "drop rindex " + indexname_pin.ToUpper();
                cmd.ExecuteNonQuery();
                conn.Close();
            }
        }
