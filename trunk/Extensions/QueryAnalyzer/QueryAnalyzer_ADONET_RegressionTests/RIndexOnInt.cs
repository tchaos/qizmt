...
taking a break.".Split(new string[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnInt()
        {
            ...
            testrows = RIndexOnInt_PrepareSourceTable(tablename, TESTSIZE);
            ...
            {
                Console.WriteLine("Creating RIndexes on key column = int...");
                DbConnection conn = fact.CreateConnection();
                conn.ConnectionString = "Data Source = localhost";
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname + " FROM " + tablenameSorted + " ON NUM1";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname_pin + " FROM " + tablenameSorted + " PinMemory ON NUM1";
                cmd.ExecuteNonQuery();
                conn.Close();
