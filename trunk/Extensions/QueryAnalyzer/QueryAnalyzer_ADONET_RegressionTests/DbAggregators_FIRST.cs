...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        public static void DbAggregators_FIRST(string tablename)
        {
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine("Testing FIRST(datetime)...");
                        DbCommand cmd = conn.CreateCommand();
                        cmd.CommandText = "select id, FIRST(bday) from " + tablename + " group by id";
                        ...
                        while (reader.Read())
                        {
                            ...
                            {
                                throw new Exception("id returned from first group by is invalid.");
                            }
                            bool found = false;
                            ...
                            if (!found)
                            {
                                throw new Exception("Invalid first received: " + first.ToString() + " instead.");
                            }
