...
namespace RegressionTest2
{
    public partial class Program
    {
        ...
        static void CacheNoFailover(string[] args)
        {
            ...
            {
                throw new Exception("This test needs at least 2 machines");
            }
...
").Replace('`', '"'));
                    ...
                    Console.WriteLine("Disrupt a machine to invoke failover...");
                    {
                    ...
                    try
                    {
                        ...
                        {
                            Console.WriteLine("Running job to use cache and invoke failover");
                            bool ok = false;
                            string output = "";
                            try
                            {
                                output = Exec.Shell("Qizmt exec Cacher");
                            }
                            catch
                            {
                                ok = true;
                            ...
                            if (!ok)
                            {
                                Console.Error.WriteLine("Expected failover with cache to abort; instead output: {0}", output);
                                throw new Exception("Expected failover with cache to abort");
                            }
                    ...
                    finally
                    {
                        {
                            string badmachine = allmachines[allmachines.Length - 2];
                            Console.WriteLine("Repairing disrupted machine");
                            Exec.Shell(@"sc \\" + badmachine + @" start DistributedObjects");
                        }
                    }
