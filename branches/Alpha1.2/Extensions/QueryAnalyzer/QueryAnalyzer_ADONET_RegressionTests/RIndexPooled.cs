...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        public static void RIndexPooled()
        {
            ...
            string indexname = Guid.NewGuid().ToString().Replace("-", "");           
            ...
            Console.WriteLine("Querying data using RSELECT...");
            {
                ...
                foreach (long key in expected.Keys)
                {
                    ...
                    cmd.CommandText = "rselect * from " + indexname + " where key = " + key.ToString();
                    DbDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        long num1 = reader.GetInt64(0);
            ...
            Console.WriteLine("RIndexPooled completed.");
            Console.WriteLine("RIndexPooled passed");
