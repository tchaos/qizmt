...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        private static OneRow[] RIndexOnIntPinHash_PrepareSourceTable(string sourcetablename, int testsize)
        {
...
taking a break.".Split(new string[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnIntPinHash()
        {
            ...
            testrows = RIndexOnIntPinHash_PrepareSourceTable(tablename, TESTSIZE);
...

            {
                ...
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname + " FROM " + tablenameSorted + " PINMEMORYHASH ON NUM1";
                cmd.ExecuteNonQuery();
                conn.Close();
