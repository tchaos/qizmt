...
namespace QueryAnalyzer_ADONET_RegressionTests
{
    ...
    {
        private static OneRow[] RIndexOnDouble_PrepareSourceTable(string sourcetablename, int testsize)
        {
...
taking a break.".Split(new string[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void RIndexOnDouble()
        {
            ...
            {
                Console.WriteLine("Creating RIndexes on key column = double...");
                DbConnection conn = fact.CreateConnection();
                conn.ConnectionString = "Data Source = localhost";
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname + " FROM " + tablenameSorted + " ON NUM2";
                cmd.ExecuteNonQuery();
                conn.Close();
...

            {
                ...
                conn.Open();
                DbCommand cmd = conn.CreateCommand();
                cmd.CommandText = "CREATE RINDEX " + indexname_pin + " FROM " + tablenameSorted + " PinMemory ON NUM2";
                cmd.ExecuteNonQuery();
                conn.Close();
